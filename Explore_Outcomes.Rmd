...
for (i in seq_along(list_files)){
  df<-read_csv(paste0('KDDEvents/', list_files[i]), col_types = cols(.default = 'c'))
...
events2<-events%>%
  #basid---------------------------------------
  mutate(cowid = paste0(ID, BDAT, EID))%>%
  mutate(date = lubridate::mdy(Date), 
         edat = lubridate::mdy(EDAT), 
         bdat = lubridate::mdy(BDAT))%>%
  mutate(floor_date_month = floor_date(date, unit = 'month'))%>%
  group_by(ID, cowid, bdat)%>%
  mutate(max_events = sum(n()))%>%
  ungroup()%>%
  #filter(max_events>81)%>%#test calvf
  #filter(ID %in% "10028537")%>%
  ...
  Event %in% c ("BLOAT", "DIPTHRA", "FEVER", "HLTHSCR", "ILLMISC", "INDIG", "INJURY", 
                "LAME", "MAST", "NAVEL", "OTHER","OTITIS",  "PINKEYE", "PNEU", "RP", 
                "SCOURS",  "SEPTIC", "SWLNLEG")~'health', 
  Event %in% c("ARVDPOT", "DNB", "DRY", "GOAMISH", "GODEER", "GOHOME", "MEASURE", "MOVE")~'mgmt', 
  Event %in% c("ARVKDD", "BEEF", "DIED", "FRESH", "SOLD")~'param', 
  Event %in% c("ASSIST", "CHECK", "CHKDBN", "CLFSERU", "COMMENT", "INV", "INWEIGH", "OBSERVE", "TBTEST",  "TP", 
               "TRIAL", "USER6", "USER7", "VACC", "XID", "ZPRBRED")~'ask', 
    Event %in% c("BANGVAC")~'vac', 
  TRUE~'Unknown'
   ))%>%
  #count events ----------------------
  ...
  ungroup()%>%
  group_by(cowid, Event)%>%
  mutate(max_event_ct = max(event_ct))%>%
  ungroup()%>%
  group_by(cowid)%>%
  mutate(list_params = paste0(unique(Event), collapse = ','))%>%
  ungroup()%>%
  mutate(outcome_basic = case_when(
    (event_ct < max_event_ct)~'Retreat', 
    (max_event_ct==1)~'Success',
    TRUE~'Other'
  ))
...
# ggplot(events2)+
#   geom_point(aes(x = date, y = Event, color = EVT_type))
